...
⚠⚠ 코드 작성 후 우측 하단 초록 화살표를 누르면 테스트 해볼 수 있고 벌레모양을 누르면 디버깅과 테스트를 동시에 진행 할 수 있다.
 ...
 ------------------------
⚠⚠publishing to tomcat ~ has encountered a problem 오류는 실행하려는 프로젝트를 우클릭 한 뒤 refresh를 클릭해주자!! (f5키를 자주 눌러주자)
...
main.jsp에서 나오는 다른 깃아이오를 보면
캘린더는 다른 깃아이오에서 api를 받아와 사용 중 이였다.
어떤식으로 돌아가는지 알고 싶다면 main.jsp에서 깃아이오에 들어가 참고 문헌을 쫌 들여다 봐야 할 듯 하다.
